...
namespace FM.Services
{
    public class PostService : IPostService
    {
        ...
        public async Task<bool> DeleteTagAsync(string tagName)
        {
            ...
            if (tag == null)
                return true;
            var temp = await _dataContext.PostTags.ToListAsync();
        ...
        public async Task<int> CountAllPostWithUser(GetAllPostsQuery query)
        {
            int count = 0;
            if (!string.IsNullOrEmpty(query?.UserId))
            {
                count = await _dataContext.Posts.CountAsync(x => x.UserId == query.UserId);
            } else
            {
                count = await _dataContext.Posts.CountAsync();
            }
            return count;
        }
    }
